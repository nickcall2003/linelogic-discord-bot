...
CHANNEL_IDS = {
    ...
    # nightly ATP/WTA summary; falls back to the daily-model channel if unset
    "tennis_digest": int(os.environ.get("CHANNEL_TENNIS_DIGEST")
                         or os.environ.get("CHANNEL_DAILY_MODEL") or "0"),
}
...
PUBLIC_COMMANDS = {
    c.strip().lower()
    for c in os.environ.get("PUBLIC_COMMANDS", "today,record,room,cappers,tournament,ladder").split(",")
    if c.strip()
}
...
# ---------------------------------------------------------------------------
# You can't fix "spammed commands cost me money" without knowing WHICH commands
# and how often. This records every invocation, flags the ones that hit the LLM
# backend (the expensive ones), and counts errors separately (a mistyped or
# failed command still cost a round trip). Persisted to disk so a redeploy
# doesn't wipe the picture. Read it with /botusage.
import json as _json
from collections import defaultdict as _dd

# Commands whose backend route calls the LLM — these are what actually cost you.
_AI_COMMANDS = {"explain", "why", "sources", "confidence", "tweet", "writeup", "model"}
_USAGE_PATH = os.environ.get("USAGE_STATS_PATH", "usage_stats.json")
_usage = {"commands": _dd(int), "ai_calls": _dd(int), "errors": _dd(int),
          "by_user": _dd(int), "since": None}
...
def _usage_load():
    ...
    try:
        with open(_USAGE_PATH) as f:
            d = _json.load(f)
        _usage["commands"] = _dd(int, d.get("commands", {}))
        _usage["ai_calls"] = _dd(int, d.get("ai_calls", {}))
        _usage["errors"] = _dd(int, d.get("errors", {}))
        _usage["by_user"] = _dd(int, d.get("by_user", {}))
        _usage["since"] = d.get("since")
    except Exception:
        pass
    if not _usage["since"]:
        _usage["since"] = now_utc().isoformat()


def _usage_save():
    try:
        with open(_USAGE_PATH, "w") as f:
            _json.dump({k: dict(v) if isinstance(v, dict) else v
                        for k, v in _usage.items()}, f)
    except Exception:
        pass


def _usage_record(cmd, user_id, errored=False):
    _usage["commands"][cmd] += 1
    if cmd in _AI_COMMANDS:
        _usage["ai_calls"][cmd] += 1
    if errored:
        _usage["errors"][cmd] += 1
    if user_id:
        _usage["by_user"][str(user_id)] += 1
    # save periodically rather than every call
    if sum(_usage["commands"].values()) % 10 == 0:
        _usage_save()


@bot.event
async def on_app_command_completion(interaction, command):
    """Fires once per SUCCESSFUL slash command — the clean place to count usage
    without editing all 30 command bodies."""
    try:
        _usage_record(command.name, getattr(interaction.user, "id", None))
    except Exception:
        pass
...
async def on_app_command_error(interaction: discord.Interaction, error):
    ...
    # record the failed invocation — a mistyped/failed command still cost a
    # round trip, and separating errors shows which commands confuse people
    try:
        cmd = interaction.command.name if interaction.command else "unknown"
        _usage_record(cmd, getattr(interaction.user, "id", None), errored=True)
    except Exception:
        pass
    try:
...
async def post_json(session: aiohttp.ClientSession, path: str, body: dict | None = None, timeout: int = 15):
    url = f"{LINE_LOGIC_API_BASE}{path}"
    headers = {"Authorization": f"Bearer {LINE_LOGIC_API_KEY}"} if LINE_LOGIC_API_KEY else {}
    async with session.post(url, json=body or {}, headers=headers, timeout=aiohttp.ClientTimeout(total=timeout)) as resp:
        resp.raise_for_status()
        return await resp.json()
...
# ---------------------------------------------------------------------------
# Nightly tennis digest
# ---------------------------------------------------------------------------
# The whale alerts are real-time and event-driven — they fire the moment an edge
# crosses the board, which is the point. This is the complement: one scheduled
# post at midnight CT with every tennis play for the day, so the tennis-following
# members get a predictable daily summary instead of only catching whatever
# happened to alert while they were watching. Tour filter matches the site — the
# headline tours only, since Challenger/ITF lines are the noise we just stopped
# alerting on.
TENNIS_DIGEST_ENABLED = os.environ.get("TENNIS_DIGEST_ENABLED", "1") == "1"
TENNIS_DIGEST_HOUR_UTC = int(os.environ.get("TENNIS_DIGEST_HOUR_UTC", "6"))  # 6 UTC = 12am CT
TENNIS_DIGEST_TOURS = {
    t.strip().upper()
    for t in os.environ.get("TENNIS_DIGEST_TOURS", "ATP,WTA").split(",")
    if t.strip()
}


async def build_tennis_digest():
    async with aiohttp.ClientSession() as session:
        try:
            data = await fetch_json(session, "/api/picks/quick",
                                    params={"sport": "tennis"}, timeout=25)
        except Exception:
            log.warning("tennis digest: board fetch failed")
            return None
    plays = []
    for p in (data.get("picks") or []):
        ...
        # only the counted tours, and skip matches already underway
        if TENNIS_DIGEST_TOURS and tier and tier not in TENNIS_DIGEST_TOURS:
            continue
        if _already_started(p):
            continue
        if p.get("edge_pct") is None or p.get("market_odds") is None:
            continue
        plays.append(p)
    if not plays:
        return None
    plays.sort(key=lambda x: x.get("edge_pct", 0), reverse=True)

    embed = discord.Embed(
        title="\U0001F3BE Today's Tennis Board",
        description=f"{len(plays)} ATP/WTA play{'s' if len(plays) != 1 else ''} "
                    f"the model likes today. Edges shown are pre-match.",
    ...
    for p in plays[:12]:
        ...
        pct = round((prob or 0) * 100) if prob is not None else None
        val = []
        if pct is not None:
            val.append(f"Model {pct}%")
        if mkt is not None:
            val.append(f"mkt {'+' if mkt > 0 else ''}{mkt}")
        if isinstance(edge, (int, float)):
            val.append(f"**+{edge:.1f}%** edge")
        name = p.get("pick", "—")
        if p.get("match"):
            name += f" — {p['match']}"
        embed.add_field(name=name[:250], value=" · ".join(val) or "—", inline=False)
    if len(plays) > 12:
        embed.set_footer(text=f"+{len(plays) - 12} more on thelinelogic.com · ATP/WTA only")
    else:
        embed.set_footer(text="Challenger/ITF shown on site but not tracked · thelinelogic.com")
    return embed


@tasks.loop(time=dtime(hour=TENNIS_DIGEST_HOUR_UTC, minute=0, tzinfo=timezone.utc))
async def tennis_digest_loop():
    if not TENNIS_DIGEST_ENABLED:
        return
    # prefer a dedicated tennis channel, fall back to the daily-model channel
    cid = (CHANNEL_IDS.get("tennis_digest") or CHANNEL_IDS.get("daily_model"))
    channel = await _resolve_channel(cid)
    if channel is None:
        return
    embed = await build_tennis_digest()
    if embed is None:
        log.info("tennis digest: no qualifying ATP/WTA plays today")
        return
    try:
        role = role_mention("tennis")
        await channel.send(content=role or None, embed=embed)
        log.info("tennis digest posted")
    except Exception as e:
        log.warning("tennis digest post failed: %s", e)
...
def _already_started(p) -> bool:
    """True if the match has begun (or begins within a 2-min grace window).

    Alerting on a game that's already underway is worse than not alerting: the
    line has moved, the model's pre-game number is stale, and it reads as the
    bot spamming dead events. The board occasionally keeps a just-started game
    listed, so this is the gate that keeps those out of the alert channels."""
    ts = p.get("start_ts")
    if not ts:
        return False                     # unknown start -> don't block (fail open)
    try:
        ...
        return False
    return datetime.now(timezone.utc) >= start + timedelta(minutes=2)
...
async def scan_edge_alerts() -> int:
    ...
    for p in (data.get("picks") or []):
        ...
        if tier and tier in EDGE_EXCLUDE_TIERS:
            skipped_tier += 1
            continue
        # never alert on a match that's already underway — the pre-game edge is
        # stale the moment the first point is played
        if _already_started(p):
            continue
        key = _edge_key(p)
        if key in seen:
...
@bot.tree.command(name="tournament", description="The tournament leaderboard — standings for the active Capper Showdown")
@app_commands.checks.cooldown(8, 60.0, key=lambda i: i.user.id)  # db read
@app_commands.describe(sort="Rank by units won (default) or win %")
@app_commands.choices(sort=[
    app_commands.Choice(name="Units", value="units"),
...
async def tournament_command(interaction: discord.Interaction,
    ...
    async with aiohttp.ClientSession() as session:
        try:
            data = await fetch_json(session, "/api/capper/tournament",
                                    params={"sort": sort_val}, timeout=20)
        except Exception:
            log.exception("tournament failed")
            await interaction.followup.send("Couldn't load the tournament board right now — try again shortly.")
            return

    meta = data.get("tournament") or {}
    tname = meta.get("name") or "Capper Showdown"
    cappers = data.get("cappers", [])
    if not cappers:
        building = data.get("building", 0)
        msg = f"No ranked entries in **{tname}** yet — standings appear once picks are graded."
        if building:
            msg += f" ({building} building a record.)"
        await interaction.followup.send(msg)
        return
    ...
    embed = discord.Embed(
        title=f"🏆 {tname} — Leaderboard",
        description="\n".join(lines),
        color=BRAND_COLOR,
        timestamp=now_utc(),
    )
    label = "units won" if sort_val == "units" else "win %"
    foot = f"Ranked by {label}"
    start = (meta.get("start_at") or "")[:10]
    if start:
        foot += f" • since {start}"
    end = (meta.get("end_at") or "")
    if end:
        foot += f" • ends {end[:10]}"
    if data.get("building"):
        foot += f" • {data['building']} still building"
    embed.set_footer(text=foot)
    await interaction.followup.send(embed=embed)


@bot.tree.command(name="tournamentreset", description="(Staff) Start a fresh tournament — resets the leaderboard window to now")
@app_commands.describe(name="Optional name, e.g. 'August Showdown'",
                       ends="Optional end date (ISO), e.g. 2026-08-31")
async def tournamentreset_command(interaction: discord.Interaction,
                                  name: str | None = None, ends: str | None = None):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            data = await post_json(session, "/api/capper/tournament/reset", body=body, timeout=20)
        except Exception:
            log.exception("tournament reset failed")
            await interaction.followup.send("Couldn't reset the tournament — try again shortly.", ephemeral=True)
            return
    if not data.get("ok"):
        await interaction.followup.send("Reset failed on the server.", ephemeral=True)
        return
    t = data.get("tournament") or {}
    start = (t.get("start_at") or "")[:19].replace("T", " ")
    if t.get("name"):
        msg = f"✅ **{t['name']}** started — window opens **{start} UTC**"
    else:
        msg = f"✅ Tournament reset — new window opens **{start} UTC**"
    if t.get("end_at"):
        msg += f", ends **{t['end_at'][:10]}**"
    msg += ". The board is clean now; past picks stay in all-time stats."
    await interaction.followup.send(msg, ephemeral=True)
...
@bot.tree.command(name="botusage", description="(Staff) Command usage — what's being run and what's costing AI credits")
async def botusage_command(interaction: discord.Interaction):
    perms = getattr(interaction.user, "guild_permissions", None)
    if not perms or not (perms.manage_guild or perms.administrator):
        await interaction.response.send_message("That's a staff-only command.", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    cmds = _usage["commands"]
    if not cmds:
        await interaction.followup.send("No command usage recorded yet.", ephemeral=True)
        return
    total = sum(cmds.values())
    ai_total = sum(_usage["ai_calls"].values())
    err_total = sum(_usage["errors"].values())

    e = discord.Embed(
        title="Command usage",
        description=f"Since {(_usage['since'] or '')[:10]} \u00b7 **{total}** total invocations",
        color=LL.INFO if hasattr(LL, "INFO") else BRAND_COLOR,
        timestamp=now_utc(),
    )
    # most-used commands, AI ones flagged
    top = sorted(cmds.items(), key=lambda kv: kv[1], reverse=True)[:12]
    lines = []
    for name, n in top:
        tag = " \U0001F9E0" if name in _AI_COMMANDS else ""      # brain = costs AI
        errs = _usage["errors"].get(name, 0)
        etag = f" \u00b7 {errs} err" if errs else ""
        lines.append(f"`/{name}`{tag} \u2014 **{n}**{etag}")
    e.add_field(name="Most used (\U0001F9E0 = hits the AI)", value="\n".join(lines), inline=False)

    e.add_field(name="AI-backed calls",
                value=f"**{ai_total}** of {total} ({round(100*ai_total/total)}%) \u2014 these are what cost money",
                inline=True)
    e.add_field(name="Errored / mistyped",
                value=f"**{err_total}** ({round(100*err_total/total)}%)",
                inline=True)

    # heaviest users — spot a single spammer
    users = sorted(_usage["by_user"].items(), key=lambda kv: kv[1], reverse=True)[:3]
    if users:
        u = "\n".join(f"<@{uid}> \u2014 {n}" for uid, n in users)
        e.add_field(name="Heaviest users", value=u, inline=False)

    e.set_footer(text="Resets only if usage_stats.json is deleted \u00b7 /botusage")
...
async def on_ready():
    ...
    if (TENNIS_DIGEST_ENABLED and CHANNEL_IDS.get("tennis_digest")
            and not tennis_digest_loop.is_running()):
        tennis_digest_loop.start()
        log.info("Tennis digest scheduled for %02d:00 UTC (ATP/WTA)", TENNIS_DIGEST_HOUR_UTC)
